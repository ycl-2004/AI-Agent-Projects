...
def arxiv_paper_search(query: str) -> str:
    ...
    """
    progress.emit({
        "type": "tool", "stage": "research",
        "message": f"  [Real Tool: arXiv] 正在真实检索学术论文库 -> '{query}'",
        "data": {"tool": "arxiv", "query": query},
    })
    try:
        import arxiv
        client = arxiv.Client()
        ...
        progress.emit({
            "type": "tool", "stage": "research",
            "message": f"  [Real Tool: arXiv] 检索遇到网络波动: {e}",
            "data": {"tool": "arxiv", "query": query, "error": str(e)},
        })
...
def wikipedia_search(query: str) -> str:
    ...
    progress.emit({
        "type": "tool", "stage": "research",
        "message": f"  [Real Tool: Wikipedia] 正在真实检索维基百科 -> '{query}'",
        "data": {"tool": "wikipedia", "query": query},
    })
    try:
        import wikipedia
        ...
        progress.emit({
            "type": "tool", "stage": "research",
            "message": f"  [Real Tool: Wikipedia] 检索遇到网络波动: {e}",
            "data": {"tool": "wikipedia", "query": query, "error": str(e)},
        })
...
def web_search(query: str) -> str:
    ...
    progress.emit({
        "type": "tool", "stage": "research",
        "message": f"  [Real Tool: Web Search] 正在真实检索互联网网页 -> '{query}'",
        "data": {"tool": "web", "query": query},
    })
    try:
        ...
        progress.emit({
            "type": "tool", "stage": "research",
            "message": f"  [Real Tool: Web Search] 联网检索遇到网络波动: {e}",
            "data": {"tool": "web", "query": query, "error": str(e)},
        })
...
def save_markdown_report(content: str, filename: str = "research_report.md") -> str:
    ...
    progress.emit({
        ...
        "message": f"  [Real Tool: Save] 研报已成功落盘至真实文件 -> {target_path}",
        "data": {"filepath": str(target_path)},
    })
